...
Definition inter (l:Line) (A B:Point) :=
  ~ Incid A l /\ ~ Incid B l /\
  exists I, Incid I l /\ Bet A I B.

Axiom Bet_1_1 : forall A B C, Bet A B C -> Col A B C.

Axiom Bet_1_2: forall A B C, Bet A B C -> A <> C.

Axiom Bet_1_3: forall A B C, Bet A B C -> Bet C B A.

Axiom Bet_2 : forall A B, A <> B -> exists C, Bet A B C.

Axiom Bet_3 : forall A B C, Bet A B C -> ~ Bet B C A.

Axiom Bet_4: forall A B C l (H: ~ Col A B C), 
  Incid_L_F l (proj1_sig (Cid ( Incid_4_1 A B C H ))) ->
  ~ Incid C l -> inter l A B -> inter l A C \/ inter l B C.

Corollary Bet_diff12 : forall A B C, Bet A B C -> A <> B.
Proof.
  intros. pose proof H. 
  intro. apply Bet_1_3 in H0. 
  apply Bet_3 in H0.
  subst. tauto.
Qed.

Corollary Bet_diff23 : forall A B C, Bet A B C -> B <> C.
Proof.
  intros. apply Bet_1_3 in H. 
  apply Bet_diff12 in H. auto.   
Qed.

Corollary Bet_diff_P : forall A B C, Bet A B C -> 
  A<>B /\ A<>C /\ B<>C.
Proof.
  intros. split. eapply Bet_diff12; eauto.
  split. eapply Bet_1_2; eauto.
  eapply Bet_diff23; eauto.
Qed. 
...
Theorem Theorem4_1 : forall A B C, A<>B -> B<>C -> C<>A 
  -> Col A B C -> 
  ( Bet A C B \/ Bet B A C \/ Bet A B C ).
Proof. 
  intros A B C HAB HBC HCA H.
  destruct H as [l [HA [HB HC]]].
  destruct (classic (Bet A C B)); auto.
  destruct (classic (Bet B A C)); auto. 
  pose proof (outline l) as [D H2].
  assert (A<>D). { IST. }
  pose proof H1. apply Incid_1 in H3 as [l0[]].
  assert (B<>D). { IST. }
  apply Bet_2 in H5 as [G].
  pose proof H5.
  apply Bet_diff_P in H6 as [H6A[H6B H6C]].
  pose proof H5. apply Bet_1_1 in H6 as [l1].
  ...
  - assert(~Col A B G). { eapply unCol_t; eauto. }
    ...
    + destruct H23 as [H23A [H23B [F[]]]].
      ...
      apply Bet_1_3 in H24; tauto. }
      apply Bet_1_1 in H24 as [l'].
    ...
    + destruct H23 as [H23A [H23B [F[]]]].
      ...
      * destruct H31 as [H31A [H31B [D'[]]]].
        pose proof H25'.
        apply Bet_diff12 in H25'.
        pose proof H32 as H32'.
        apply Bet_1_1 in H32 as [l''].
        apply (line_eq G E l'' l') in H25'; try tauto.
        subst. assert(D'<>C). { intro.
        subst. apply Bet_3 in H33.
        apply Bet_1_3 in H32'. tauto. }  
...
Theorem Theorem4_2 : forall A B C,  Bet A B C ->  
  ~ Bet B C A /\ ~ Bet B A C.
Proof.
  intros. split.
  - apply Bet_3 in H. auto.
  - apply Bet_1_3 in H. 
    apply Bet_3 in H. auto.
Qed.

Lemma lem1_1_T5 : forall A B C D ,
  Bet A B C -> Bet B C D -> Bet A C D. 
Proof.
  intros. pose proof H. pose proof H0.
  apply Bet_diff_P in H1 as [H1[]]. 
  apply Bet_diff_P in H2 as [H2[]].
  pose proof H as H'.
  pose proof H0 as H0'. 
  apply Bet_1_1 in H'. apply Bet_1_1 in H0'.
  destruct H' as [l].
  destruct H0' as [l0].
  apply (line_eq B C l l0) in H4; try tauto.
  subst l0. pose proof (outline l) as [E HL].
  assert (C<>E). { IST. } 
  pose proof H4. apply Bet_2 in H9 as [F]. 
  pose proof H9. apply Bet_1_2 in H10. 
  pose proof H9. apply Bet_1_1 in H11 as [l0].
  ...
  subst. tauto. }
  assert(A<>E). { IST. }
  assert(E<>F). { apply Bet_diff23 in H9. auto. }
  ...
  - destruct H20 as [H20A[H20B[G[]]]].
    ...
    subst. tauto.
  - destruct H20 as [H20A[H20B[G[]]]].
    pose proof H21. apply Bet_diff_P in H21 as [H21[]].
    pose proof H22 as H22'.
    apply Bet_1_1 in H22 as [l'].
    assert(~ Incid G l). { intro.
    ...
    + destruct H31 as [H31A[H31B[P[]]]].
      ...
      * destruct H38 as [H38A[H38B[C'[]]]].
        ...
        -- destruct H47 as [H47A[H47B[G'[]]]].
           ...
           apply Bet_1_3 in H48. tauto. }
           apply Bet_1_1 in H48 as [l''].
           apply (line_eq C E l'' l0) in H4; try tauto.
           subst. apply (line_eq G' F l' l0) in H49; subst; tauto.
Qed.

Lemma lem1_2_T5 : forall A B C D, Bet A B C -> 
  Bet B C D -> Bet A B D.
Proof. 
  intros. apply Bet_1_3. 
  apply Bet_1_3 in H.
  eapply lem1_1_T5; eauto. 
  apply Bet_1_3; auto.
Qed.

Lemma lem1_T5 : forall A B C D, Bet A B C -> 
  Bet B C D -> Bet A C D /\ Bet A B D.
Proof.
  intros. split. eapply lem1_1_T5; eauto.
  eapply lem1_2_T5; eauto.
Qed.
 
Lemma lem2_1_T5 : forall A B C D, Bet A B C -> 
  Bet A C D -> Bet B C D.
Proof.
  intros. pose proof H. pose proof H0.
  apply Bet_diff_P in H1 as [H1[]]. 
  ...
  pose proof H as H'.
  pose proof H0 as H0'. 
  apply Bet_1_1 in H' as [l].
  apply Bet_1_1 in H0' as [l0].
  apply (line_eq A C l l0) in H2; try tauto.
  subst l0. pose proof (outline l) as [G HL].
  assert(B<>G). { IST. }
  pose proof H2. apply Bet_2 in H9 as [F].
  pose proof H9. apply Bet_1_1 in H10 as [l0].
  pose proof H9. apply Bet_diff_P in H11 as [H11A[H11B]].
  ...
  - assert(A<>G). { IST. }
    ...
    + destruct H31 as [H31A[H31B[A'[]]]].
      ...
      subst. tauto. apply Bet_1_1 in H32 as [l'].
      apply (line_eq B C l' l) in H4; try tauto.
      subst. tauto.
    + destruct H31 as [H31A[H31B[A'[]]]].
      pose proof H32. apply Bet_1_1 in H33 as [l'].
      pose proof H32. apply Bet_diff_P in H34 as [H34A[H34B]].
      ...
      * destruct H39 as [H39A[H39B[G'[]]]].
        ...
        apply Theorem4_2 in H41. apply Bet_1_3 in H40. tauto. }
        apply (line_eq A' P l1 l2) in H42; try tauto.
        subst. tauto. apply Bet_1_1 in H41 as [l'].
...
Lemma lem3_T5 : forall A B C D l, 
  ...
  (Bet A B C /\ Bet A D C)\/ (Bet A C B /\ Bet A D B) \/
  (Bet B A C /\ Bet B D C) \/ (Bet B A D /\ Bet B C D) \/ 
  (Bet C A D /\ Bet C B D) \/ (Bet A B D /\ Bet A C D).
Proof.
  intros. destruct (classic (Bet A B C /\ Bet A D C)); auto.
  apply not_and_or in H9 as [].
  - assert(Col A B C). { exists l; auto. }
    apply Theorem4_1 in H10; auto.
    destruct H10 as [H10|[H10|H10]]; try tauto.
    + destruct (classic (Bet A D B)); auto.
      ...
      * apply Bet_1_3 in H10. apply Bet_1_3 in H12.
        pose proof (lem2_T5 B C A D H10 H12). tauto. 
    + destruct (classic (Bet B D C)); auto.
  ...
  - assert(Col A D C). { exists l; auto. }
    apply Theorem4_1 in H10; auto.
    destruct H10 as [H10|[H10|H10]]; try tauto.
    + destruct (classic (Bet A B D)). auto 7.
      ...
      * apply Bet_1_3 in H10. apply Bet_1_3 in H12.
        ...
        apply Bet_1_3 in H14. 
        apply Bet_1_3 in H12. tauto. 
    + apply Bet_1_3 in H10. 
      destruct (classic (Bet C B D)); auto 7.
      assert(Col C B D). { exists l; auto. }
      apply Theorem4_1 in H12; auto.
      destruct H12 as [H12|[H12|H12]]; try tauto.
      * pose proof (lem2_T5 C A D B H10 H12) as [].
        apply Bet_1_3 in H12. apply Bet_1_3 in H14. auto.
      * apply Bet_1_3 in H10. apply Bet_1_3 in H12.
        pose proof (lem2_T5 D A C B H10 H12) as []. 
        apply Bet_1_3 in H14. apply Bet_1_3 in H12. auto 6.
Qed.
...
Proof.
  ...
  apply Bet_diff_P in H0' as [HA [HB HC]].
  destruct (classic (Bet A B C)). 
  - pose proof (lem2_T5 A B C D H2 H1). tauto.
  - assert(Col A B C). { apply Bet_1_1 in H0 as []. 
    apply Bet_1_1 in H1 as [].
...
Lemma inter_cof : forall A B l, inter l A B ->
...
Lemma inter_exists : forall A l, ~ Incid A l ->
  exists P, inter l A P.
Proof.
  intros. pose proof (Incid_3_1 l) as [B [C [H0[]]]].
  assert(A<>B). { IST. }
  apply Bet_2 in H3 as [].
  exists x. split; auto. split.
  - intro. pose proof H3.
    apply Bet_diff_P in H5 as [H5A[H5B]]. 
    apply Bet_1_1 in H3 as [l1].
    apply (line_eq B x l l1) in H5; auto; 
    subst; tauto. 
  - exists B. tauto.
Qed.

Lemma sym_same_side : forall A a, ~ Incid A a ->
  same_side_L_eq A A a.
Proof.
  intros. pose proof (inter_exists A a H) as [].
  exists x. tauto.
Qed.

Lemma Bet_Ips_P : forall A B C D, Bet A C D 
  -> Bet B C D -> ~ Bet A C B. 
Proof.
  intros.  
  pose proof H. apply Bet_diff_P in H1.
  pose proof H0. apply Bet_diff_P in H2.
  pose proof H. apply Bet_1_1 in H3 as []. 
  pose proof H0. apply Bet_1_1 in H4 as []. 
  destruct H1 as [H1 [H1' H1'']].
  apply (line_eq C D x x0) in H1''; try tauto. subst.
  assert(Col A C B). { exists x0; tauto. } 
  destruct H2 as [H2' ].
  intro. pose proof H6. apply Bet_diff_P in H7. 
  assert (Bet A B C /\ Bet A D C \/
     Bet A C B /\ Bet A D B \/
     Bet B A C /\ Bet B D C \/
     Bet B A D /\ Bet B C D \/
     Bet C A D /\ Bet C B D \/
  ...
  destruct H8.
  - apply Bet_3 in H8. apply Bet_1_3 in H6. tauto.
  - apply Bet_1_3 in H9. 
    pose proof (lem2_T5 B C D A H0 H9) as [].
    apply Theorem4_2 in H10. 
    apply Bet_1_3 in H. tauto.
  - apply Bet_3 in H9. 
    apply Bet_1_3 in H0. tauto.
  - apply Bet_1_3 in H.
    apply Bet_1_3 in H8.
...
Proof.
  ...
  - destruct (classic (B=C)). 
    + subst; auto.
    + pose proof H2. apply Bet_1_1 in H2 as [].
      pose proof H6. apply Bet_diff_P in H6. 
      destruct H6 as [H6'[]].
      destruct (classic (Bet A B C)). 
      pose proof (lem2_T5 A I B C H7 H9).
      right. repeat(split; auto). 
      * exists I; tauto.
      * destruct (classic (Bet A C B)). 
        assert(I<>C). { IST. }
...
Proof.
  ...
  apply Bet_1_3 in H2; repeat(split; auto);
  exists P; auto.
Qed.

Lemma same_side_r: forall A A' a f,
  same_side_L A A' a f ->
  same_side_L_eq A A' a.
Proof.
  intros. destruct H as [H[H0[H1[H2 []]]]].
  pose proof (inter_exists A a H2).
  destruct H5 as [B]. 
  destruct (classic(Col A B A')).
  - pose proof (Col_inter A B A' a H6 H3 H5).
    destruct H7. red. exists B. tauto. tauto.
  - exists B. split; auto.
    assert(inter a A A' \/ inter a B A') as [].
    { apply Bet_4 with H6; auto.
    apply inter_cof in H5.
    destruct H5 as [f1]. 
    pose proof (Theorem2_1 a A H2).
    destruct H7 as [f2[]].
    assert(f2=f). { apply H8; auto. }
    assert(f2=f1). { apply H8; tauto. } subst. 
    pose proof ((proj2_sig (Cid (Incid_4_1 A B A' H6)))) as [].
    set (proj1_sig (Cid (Incid_4_1 A B A' H6))) as f.
    assert(f=f1). { apply H10. tauto. }
    subst. auto. }
    tauto. apply exc_diff_side_L. tauto.
Qed.
...
Lemma lem_inter_Bet_4 : forall A B C L M N l, 
  ~ Col A B C -> ~ Incid A l -> ~ Incid B l -> 
  ~ Incid C l ->  Incid L l ->  Incid N l -> Incid M l ->
  Bet L N M -> Bet A L B -> Bet A N C -> 
  Bet B M C -> False.
Proof.
  intros. assert(~Col L M B). { 
  eapply unCol_t; eauto.
  apply Bet_1_2 in H6. auto. }
  assert(A<>L) as HAL. { IST. } 
  pose proof H7. 
  apply Bet_1_1 in H11 as [l1].
  pose proof H8.
  apply Bet_1_1 in H12 as [l0].
  pose proof H9. 
  apply Bet_1_1 in H13 as [l2].
...
Lemma inter_Bet_4 : forall A B C l,
  ~ Col A B C -> inter l A B ->
  inter l A C -> ~ inter l B C.
Proof.
  intros. intro.
  ...
  pose proof H4 as H4'.
  pose proof H5 as H5'.
  apply Bet_1_1 in H3 as [].
  apply Bet_1_1 in H4 as [].
  apply Bet_1_1 in H5 as [].
  ...
  subst. apply H. exists x1; tauto. }
  assert( Bet L N M \/ Bet M L N \/ Bet L M N).
  { apply Theorem4_1; auto. exists l; auto. }
  destruct H12 as [H12|[H12|H12]].
  - apply (lem_inter_Bet_4 A B C L M N l); auto. 
  - apply (lem_inter_Bet_4 B C A M N L l); auto; 
    try (apply Bet_1_3; auto). n_Col_P. 
  - apply (lem_inter_Bet_4 B A C L N M l); auto;
    try (apply Bet_1_3; auto). n_Col_P.
Qed. 
...
Proof.
  ...
  - pose proof (Col_notinter P A A' a H5 H1 H2).
    tauto.  
  - pose proof (inter_Bet_4 P A A' a H5 H1 H2).
    tauto. 
Qed.
